...
@dataclass
class ZoneEnvWalls(Geom):  # pylint: disable=too-many-instance-attributes
    """
    The walls at the boundary of any ZoneEnv tasks.
    """

    name: str = 'zone_env_walls'
    num: int = 4
    ...
    def cal_cost(self):
        ...
        cost = {'wall_sensor': self.wall_sensor(x, y), 'cost_zone_env_walls': 0}
        if (
            x >= self.collision_threshold
            or x <= -self.collision_threshold
            or y >= self.collision_threshold
            or y <= -self.collision_threshold
        ):
            cost['cost_zone_env_walls'] = 1
        return cost
